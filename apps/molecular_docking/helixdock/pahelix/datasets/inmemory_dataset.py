...
class InMemoryDataset(object):
    ...
    def _load_npz_data_files(self, data_files):
        ...
        for f in data_files:
            try:
                ...
                print("[NPZ_loading_failed] ", f)
                fc += 1
        print(f"[failed_count] {fc}/{len(data_files)}")
        return data_list

    def _save_npz_data(self, data_list, data_path, max_num_per_file=2000):
    ...
    def save_data(self, data_path, max_num_per_file=2000):
        """
        ...
        Args:
            data_path(str): the path to the cached npz path.
        """
        self._save_npz_data(self.data_list, data_path, max_num_per_file=max_num_per_file)
    ...
    def transform(self, transform_fn, num_workers=4, drop_none=False):
        ...
        print('transform done start drop none')
        if drop_none:
