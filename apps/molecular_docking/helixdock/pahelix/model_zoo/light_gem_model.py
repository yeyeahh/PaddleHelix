...
class LiteGEMConv(paddle.nn.Layer):
    """tbd"""
    def __init__(self, config, with_efeat=True, update='node'):
        ...
        self.emb_dim = self.config["graph_feat_size"]
        ...
        if self.concat:
            ...
            self.fc_concat_edge = Linear(self.emb_dim * 3, self.emb_dim)
        ...
        self.mlp_edge = MLP(channels_list,
                       norm=self.config["norm"],
                       last_lin=True)
    ...
    def send_func_edge(self, src_feat, dst_feat, edge_feat):
        """tbd"""
        h = paddle.concat([dst_feat['h'], src_feat['h'], edge_feat['e']], axis=1)
        h = self.fc_concat_edge(h)
    ...
    def forward(self, graph, nfeat, efeat):
        ...
        if self.update == 'node':
            ...
            out = self.mlp(out)
            return out, efeat
        elif self.update == 'edge':
            msg = graph.send(src_feat={"h": nfeat},
                            dst_feat={"h": nfeat},
                            edge_feat={"e": efeat},
                            message_func=self.send_func_edge)
            out = msg['h']
            out = efeat + out
            out = self.mlp_edge(out)
            return nfeat, out
