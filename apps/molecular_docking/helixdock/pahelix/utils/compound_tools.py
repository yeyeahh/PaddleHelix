...
class CompoundKit(object):
    ...
    def atom_to_feat_vector(atom):
        ...
        atom_names = {
            ...
            "total_numHs": safe_index(CompoundKit.atom_vocab_dict["total_numHs"], atom.GetTotalNumHs(includeNeighbors=True)),
...
def new_mol_to_graph_data(mol, if_fingerprint=False):
    """
    mol_to_graph_data
...
def mol_to_transformer_data(mol):
    """tbd"""
    if len(mol.GetAtoms()) == 0:
        return None
    data = mol_to_graph_data(mol)
    data['dist_matrix'] = Chem.rdmolops.GetDistanceMatrix(mol)
    return data

def get_path_mask(path):
    """
    path : (N , N, MAX_HOP)
    path_mask: (N, N, N)
    """
    N, _, E = path.shape
    path_mask = np.zeros([N, N, N])
    ...
    for i in range(N):
        for j in range(N):
            if i == j:
                path[i][j] = [i, -1, -1]
            for k in range(E):
                node_index = path[i][j][k]
                if node_index >= 0:
                    path_mask[i][j][node_index] = 1
    #loop_time = time.time()
    #path_mask2 = np.zeros([N, N, N])
    # for k in range(E):
    #     path_mask2 += np.eye(N)[path[:, :, k]]
    # eye_time = time.time()
    # print(' path mask : ', path_mask)
    # print(' eye path mask : ', path_mask2)
    # for-loop using around 5 times the time compared to eye, but eye cannot handle the -1 correctly
    # print('loop time : {} , eye time : {}'.format(loop_time - s_time, eye_time - loop_time))
    return path_mask


def mol_to_trans_data_w_meta_path(mol, max_hop):
    """
    get meta_path up to max_hop
    """
    import pyximport
    pyximport.install(setup_args={'include_dirs': np.get_include()})
    from pahelix.utils import algos
    N = len(mol.GetAtoms())
    if N == 0:
        return None
    data = mol_to_graph_data(mol)
    ...
    edge_mask = np.zeros([N, N])
    edges = data['edges']
    edge_mask[edges[:, 0], edges[:, 1]] = 1
    shortest_path_result, path = algos.floyd_warshall(edge_mask.astype('int64')) # path:(N, N)
...
def mol_to_trans_data_w_rdkit3d(mol):
    """
    get rdkit3d positions
    """
    N = len(mol.GetAtoms())
    if N == 0:
        return None
    ...
    _, atom_pos = Compound3DKit.get_MMFF_atom_poses(mol, numConfs=40)
    data['atom_pos'] = np.array(atom_pos, 'float32')
    return data


def mol_to_trans_data_w_pair_dist(mol):
    """
    tbd
    """
    N = len(mol.GetAtoms())
    ...
    try:
        atom_poses = Compound3DKit.get_atom_poses(mol, mol.GetConformer())
    # data['bond_length'] = Compound3DKit.get_bond_lengths(data['edges'], atom_poses)
        data['pair_dist'] = Compound3DKit.get_pair_distances(N, np.array(atom_poses))
    except:
        data['pair_dist'] = np.ones([N, N]) * np.nan
    return data
