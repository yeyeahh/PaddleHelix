...
def get_all_path(path, max_dist):
    (nrows, ncols) = path.shape
    assert nrows == ncols
    cdef unsigned int n = nrows
    cdef unsigned int max_dist_copy = max_dist
    path_copy = path.astype(long, order='C', casting='safe', copy=True)
    cdef numpy.ndarray[long, ndim=3, mode='c'] all_path = -1 * numpy.ones([n, n, max_dist_copy + 1], dtype=numpy.int64)
    ...
    for i in range(n):
        for j in range(n):
            ...
            if path_copy[i][j] == 510:
                continue
            path = [i] + get_all_edges(path_copy, i, j) + [j] # (L)
            all_path[i][j][:len(path)] = path
    return all_path
