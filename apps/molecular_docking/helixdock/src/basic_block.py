...
class RBFEmbedding(nn.Layer):
    ...
    def __init__(self, names, embed_dim, rbf_params=None):
        ...
        for name in self.names:
            rbf_param = rbf_params[name]
            if 'centers' in rbf_param:
                centers = rbf_param['centers']
            else:
                centers = np.arange(rbf_param['start'], rbf_param['end'], rbf_param['stride'])
            gamma = rbf_param['gamma']
...
class MultiPosRBFEmbedding(nn.Layer):
    """
    MultiPosRBFEmbedding
    rbf_params: 
        {
            name: {
                'centers': (num_center),
    ...
    """
    def __init__(self, names, embed_dim, rbf_params=None):
        super(MultiPosRBFEmbedding, self).__init__()
        self.names = names
        ...
        for name in self.names:
            ...
            gamma = rbf_param['gamma']
            self.rbf_dict[name] = RBF(centers, gamma)
            self.linear_dict[name] = nn.Linear(self.pos_num * len(centers), embed_dim)
    ...
    def forward(self, feats_list):
        ...
        for name in self.names:
            ...
            rbf_x_concat = paddle.concat(rbf_x_list, axis=-1)
            out_embed += self.linear_dict[name](rbf_x_concat)
        return out_embed
...
def get_vec_norm(vec, keepdim=False):
    ...
    vec: (*, d) -> (*, 1) or (*)
    """
    return paddle.sqrt(paddle.sum(paddle.square(vec), -1, keepdim=keepdim) + 1e-5)
...
def ligand_protein_pos_to_pair_dist(liagnd_pos, protein_pos):
    """
    atom_pos: (b, n, 3)
    return:
        dist: (b, n, n)
    """
    left = liagnd_pos.unsqueeze([2])  # (b, n, 1, 3)
    right = protein_pos.unsqueeze([1]) # (b, 1, n', 3)
    dist = get_vec_norm(left - right)  # (b, n, n')
    return dist
...
def atom_pos_to_center_dist(atom_pos, node_mask):
    """
    atom_pos: (b, n, 3)
    node_mask: (b, n)
    return:
    ...
    center = (atom_pos * node_mask).sum(1) / node_mask.sum(1)   # (b, 3)
    vec = atom_pos - center.unsqueeze([1])  # (b, n, 3)
    dist = get_vec_norm(vec)  # (b, n)
    return dist


def cast_to_bucket(value, bin_start, bin_end, num_bins):
    assert bin_start < bin_end, (bin_start, bin_end)
    assert num_bins > 0
    stride = float(bin_end - bin_start) / num_bins
    bin_id = paddle.cast((value - bin_start) / stride, 'int64')
    bin_id = paddle.clip(bin_id, 0, num_bins - 1)
    return bin_id